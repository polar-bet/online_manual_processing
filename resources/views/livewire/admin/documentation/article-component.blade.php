        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Таблиця зі статтями документації</h3>
                    ...
                    <table class="table table-hover text-nowrap">
                        ...
                        @foreach($articles as $article)
                            <tr>
                                <td>{{$article->id}}</td>
                                <td>{{$article->name}}</td>
                                <td>{{$article->documentationType->name}}</td>
                                <td>{{$article->documentationType->documentationSection->name}}</td>
                                <td>{{$article->created_at}}</td>
                                <td>
                                    <form action="{{route('admin.documentation.article.delete', $article->id)}}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <div class="btn-group">
                                            <a href="{{route('admin.documentation.article.show', $article->id)}}"
